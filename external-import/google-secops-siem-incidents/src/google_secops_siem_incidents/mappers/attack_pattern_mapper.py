...
def map_attack_patterns(
    ...
    *,
    ...
    if attack_pattern_outcome is None:
        return []
    if attack_pattern_outcome.string_seq is None and attack_pattern_outcome.string_val is None:
        return []

    if attack_pattern_outcome.string_val:
        attack_pattern = attack_pattern_outcome.string_val
        result.append(AttackPattern(name=attack_pattern, mitre_id=attack_pattern, author=author, markings=[tlp_marking]))
